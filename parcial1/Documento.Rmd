...
##Histograms
###We can se in the histogram that the distribution favors to the left wich makes sense. this must be the comercial cpu's and to the rigth we see cpu's that where developed for more specialiced purposes
```{r}
...
hist(cpuData$CHMAX, xlab = 'Max cache in kb',main = 'Histogram of max cache in kb', border = 'black',col = 'orange' )
hist(cpuData$CHMIN, xlab = 'Min cache in kb',main = 'Histogram of min cache in kb', border = 'black',col = 'orange' )

```
###Histogrm of machine cycle in nanoseconds
```{r}
...
head(sortedCpuData)
```
###Next let's obtain the quartiles and plot them
MACHINE CYCLE
whe can see the minimum value is 17 while the maximum value is 1500 and the median 110
...
boxplot(sortedCpuData$MYCT, horizontal = TRUE)
```

MIN MEMORY IN KB 
whe can see the minimum value is 64 while the maximum value is 32000 and the median 2000
...
boxplot(sortedCpuData$MMIN, horizontal = TRUE)
```

MAX MEMORY IN KB 
whe can see the minimum value is 64 while the maximum value is 64000 and the median 8000
...
boxplot(sortedCpuData$MMAX, horizontal = TRUE)
```

PUBLICATED PERFORMANCE 
whe can see the minimum value is 6 while the maximum value is 1150 kb/nanoseconds and the median 50 kb/nanoseconds
...
whe can see the minimum value is 15 kb/nanoseconds while the maximum value is 1238 and the median 45 kb/nanoseconds
