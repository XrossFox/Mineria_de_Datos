...
## Scatterplots de carrera (Overview grafica por carreras)
...
Se Normalizan los datos para ponerlos en una escala de 0 a 1 y sea traer los datos numericos a la misma escala y sea posible aplicar el algoritmo K-NN
```{r, echo=TRUE}
#reinicio de datos
alumnos <-read.csv("dataset-alumnos-v2.csv")
#Asignacion al azar
...
##Conclusion
We can conclude that for the correlation we may need way more data for more
precise results, and this can be noted no the scatterplots.
This results are really likely not representative of the actual population
and it's defects may have to do with students not being that honest.

##Pros
With more precise results this could help us predict what carrer it is
more likely to be picked by a student, or maybe find students that maybe 
they may have not picked a carrer more suited for them.